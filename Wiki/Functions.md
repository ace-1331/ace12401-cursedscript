...
Beta: Functions still undergoing tests and changes during a beta, be wary of potential weird development.
...
Public: Public level of access, anyone can use it on the target
...
Club: Club owner level of access, only one exists and is the one who have her collar on your neck. Can do anything she wishes and includes every other access level except Wearer

When a function exists both as Wearer and another role, two letters will be used
...
| mistress [ID] | Setting | Makes the character with this ID a mistress regarding the curse, giving some more access | NEW |  | Wearer, Mistress |
...
| quickban | Action | Bans known recurrent trolls and your own blacklist in a room you are admin of |  |  | Wearer |
...
These functions are all about setting an user's apparent name or title
...
| deactivateonpresence [on/off] | Setting | Removes all restrictions in presence of a mistress. Note that it deactivate everything, even restrictions from owners |  |  | Mistress |
...
| onlyonpresence | Setting | Makes it so the rules are only active when an owner is in the same room as you |  |  | Owner |
...
| Function | Type | Effect | Status | Intense | Access |
| --- | --- | --- | --- | --- | --- |
| strictness [Low/Normal/Strict] | Setting | Defines how strict the punishment for breaking rules is | NEW |  | Owner |
| punishmentrestraint [1-10] [Item group] | Setting | Defines the currently worn item in that item group to a punishment level. When triggering said punishment, it will apply this item, or if none are present, the next one in descending order | NEW |  | Owner |
...
| reward | Action | For good curse wearers, this removes strikes | NEW |  | Public |
...
| clothed | Rule | Removes the rule about being naked | NEW |  | Mistress |
| afk | Rule | Punishes the victim whenever she is set AFK. Requires the option to set the AFK icon to be toggled on |  |  | Owner |
| vibes | Rule | Forces all vibes to the speed set by vibratorintensity (see below) |  |  | Mistress |
| vibratorintensity [off, low, medium, high, max] | Rule | Sets the speed for forced vibrators | NEW |  | Mistress |
| asylum [Hours] | Rule | Adds or removes a number of hours to the time to spend at the asylum |  |  | Owner |
| sendasylum | Action | Sends the wearer to the asylum | NEW |  | Owner |
...
| secretorgasm | Setting | Hides the arousal meter from the cursed person, without deactivating it | NEW | X | Owner |
...
Anything regarding how your speech is controlled is found there
...
| restrainedspeech | Rule | Defines if speech restraining rules are active. Default: ON |  | X | Owner |
...
| fullmute | Setting | Prevents the curse bearer to send normal messages altogether. Note that this is not a rule, as there are no punishments for trying |  | X | Owner |
...
| retype | Setting | The wearer will no longer need to retype the whole message when it contains a transgression | NEW |  | Owner |
...
| Function | Type | Effect | Status | Intense | Access |
| --- | --- | --- | --- | --- | --- |
| draw [Number of cards] [Player] | Action | Draw cards from a 52 cards deck. If Player is empty, it draws for you |  |  | Wearer |
...
This is because it isn't the curse saving this, but the character, being functions present in the base game. These functions are merely shortcuts for the Club owner instead of opening the interaction menu, and exists only for the looseowner option

| Function | Type | Effect | Status | Intense | Access |
| --- | --- | --- | --- | --- | --- |
| forcedlabor | Action | Send the target to work as a maid, to serve drinks in rooms |  |  | Club |
| remoteself | Setting | Prevents the use of remote on your own vibrators |  |  | Club |
| remoteblock | Setting | Prevents the use of remote on any vibrators |  |  | Club |
| unlockself | Setting | Blocks the use of keys on your own bindings |  |  | Club |
| keyblock | Setting | Removes the use of keys entirely |  |  | Club |
| blockchange | Setting | Removes access to the wardrobe entirely |  |  | Club |


# Examples of use

Not all functions are easy to make use correctly, so some example uses are given. Note that you pretty much are required to be two, except for Wearer level functions, so it could be a good idea to go in a private room to experiment, and take notes of what works for you or not, without being spammed with outside messages.
...
We assume you are using Intense mode, as the more complex functions tends to require it. You can read it still if you desire not use it.

#Target wardrobev2

This function should be used by Target. This gives access to an upgraded wardrobe, and is the typical example of a toggle function. Using it again will remove access to it, but really, it is by far the best reason to use Curse


#Target owner [ID]

Again, Target is the one using this function and gives almost full access to the other person. You need to give their ID so the final result should be like #Target owner 12345


#Target showowners

Both can use this function, it will confirm that our new owner has been set correctly. If not, repeat the previous command with the first ID to remove the wrongly added Owner
...
For this command, put a binding on Target's torso, such as a harness

#Target curseitem torso 1

The harness on Target will be cursed for one hour. Target should try to remove it to understand the effect
